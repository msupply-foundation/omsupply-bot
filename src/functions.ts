...
export const find = <T>(a: readonly T[], f: (_: T) => boolean): T | undefined => a.find(f) || undefined
...
export const merge = (a: Object[]): Object => reduce(a, (acc: Object, curr: Object) => ({ ...acc, ...curr }))

export const mapDistinct = <T, U>(a: readonly T[], f: (_: T) => U): U[] => distinct(map(a, f))

export const mapFilter = <T, U>(a: readonly T[], f: (_: T) => U, g: (_: U) => boolean): U[] => filter(map(a, f), g)

export const mapFilterNull = <T, U>(a: readonly T[], f: (_: T) => U): NonNullable<U>[] => filterNull(map(a, f))
...
export const mapReduce = <T, U>(a: readonly T[], f: (_: T) => U, g: (acc: U, curr: U) => U): U => reduce(map(a, f), g)

export const flatMapPromise = async <T, U>(a: readonly T[], f: (_: T) => Promise<U[]>): Promise<U[]> =>
    Promise.all(map(a, f)).then(flat)

export const mapPromise = async <T, U>(a: readonly T[], f: (_: T) => Promise<U>): Promise<U[]> => Promise.all(map(a, f))
