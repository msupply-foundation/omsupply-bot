...
export = (app: Application) => {
    app.on(['pull_request.opened', 'pull_request.reopened'], async (context: Context) => {
        const issueRegex = /\#(\d+)/
        const issueRegexMatch = issueRegex.exec(context.payload.pull_request.body)
        ...
        if (issueRegexMatch) {
            ...
            if (issueNumber) {
                ...
                if (projects) {
                    ...
                    if (project) {
                        const columns = await context.github.projects.listColumns({
                        ...
                        if (columns) {
                            ...
                            if (columnDoing && columnInPR) {
                                const cardsDoing = await context.github.projects.listCards({
                                    column_id: columnDoing.id,
                                })
                                if (cardsDoing) {
                                    const cardIssue = cardsDoing.data.find(card => card.content_url === issue.data.url)
                                    if (cardIssue) {
                                        const { id: card_id } = cardIssue
